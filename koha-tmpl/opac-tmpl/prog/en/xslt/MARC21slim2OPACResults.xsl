...
<xsl:text> </xsl:text> <!-- added blank space to fix font display problem, see Bug 3671 -->
    ...
    <!-- WAHT: EBSCO Journals Link -->
    <xsl:if test="marc:datafield[@tag=022]">
      <img src="/opac-tmpl/prog/images/image004.gif" alt="Journal A-Z list" title="Journal A-Z list" class="format"/>
      <span class="results_summary">
        <b>
          <span class="label">Check our Journal A-Z list for online access: </span>
        </b>
        <xsl:for-each select="marc:datafield[@tag=022]">
          <xsl:for-each select="marc:subfield[@code='a']">
            <a>
              <xsl:attribute name="href">http://atoz.ebsco.com/titles.asp?txtKeyword=<xsl:value-of select="text()"/>&amp;SF=Titles&amp;id=3081&amp;cmdSearchSubmit=Search</xsl:attribute>
