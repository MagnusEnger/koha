...
MAILADDR="hosting@ptfs-europe.com"
...
echo
echo Backup of Database Server - $HOST system id: $CUST_ID
echo ======================================================================
...
#               echo eval rm -fv "$BACKUPDIR/weekly/${DB}/${DB}_week.$REMW.*"
                eval rm -fv "$BACKUPDIR/weekly/${DB}/${DB}_week.$REMW.*"
# Place to put weekly tar rotation
eval rm -fv "$BACKUPDIR/weekly/tar_week.$REMW.*"
                echo
# Place to put weekly tar
CURDIR=`pwd`
cd /
tar -T $BACKUPDIR/dirs_to_go -czf "$BACKUPDIR/weekly/tar_week.$W.$DATE.tar.gz"
cd $CURDIR
...
then
        cat "$LOGFILE" | mail -s "MySQL Backup Log for $CUST_ID ($HOST) - $DATE" $MAILADDR
        if [ -s "$LOGERR" ]
                then
                        cat "$LOGERR" | mail -s "ERRORS REPORTED: MySQL Backup error Log for $CUST_ID ($HOST) - $DATE" $MAILADDR
        fi
