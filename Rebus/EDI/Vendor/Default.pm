...
sub parse_invoice {
    ...
    for ( $count = 0 ; $count < $message_count ; $count++ ) {
        my $dt = DateTime->new(
            year  => substr( $messages->[$count]->{message_date}, 0, 4 ),
        ...
        if ( $messages->[$count]->{allowance_or_charge} ) {
            ...
            for ( $acount = 0 ; $acount < $aoc_count ; $acount++ ) {
                if ( $messages->[$count]->{allowance_or_charge}->[$acount]
                    ->{type} eq 'C'
                    && $messages->[$count]->{allowance_or_charge}->[$acount]
                    ->{service_code} eq 'DL' )
                {
                    $delivery =
                      $delivery +
        ...
        foreach my $item ( @{$items} ) {
            ...
            if ( $item->{item_allowance_or_charge} ) {
                ...
                for ( $ccount = 0 ; $ccount < $charge_count ; $ccount++ ) {
                    if ( $item->{item_allowance_or_charge}->[$ccount]->{type} eq
                        'C' )
                    {
                        $item_charge =
                          $item_charge +
                          $item->{item_allowance_or_charge}->[$ccount]
                          ->{amount};
                    }
                }
            ...
            if ( $item->{item_tax} ) {
                ...
                for ( $tcount = 0 ; $tcount < $tax_count ; $tcount++ ) {
                    if (   $item->{item_tax}->[$tcount]->{type_code} eq 'VAT'
                        && $item->{item_tax}->[$tcount]->{rate} > 0 )
                    {
