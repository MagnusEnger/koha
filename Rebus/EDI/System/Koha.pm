...
sub get_ordernumber_from_supplier_ref {
    ...
    my $dbh = C4::Context->dbh;
    my $sth = $dbh->prepare(
        'select ordernumber from aqorders where supplierreference=?');
...
sub process_invoices {
    ...
    foreach my $invoice (@$invoices) {
        ...
        foreach my $inv ( @{$parsed_invoice} ) {
            ...
            update_invoice_status( $invoice->{logged_message_id},
                $invoice->{edi_account_id}, $invoiceid );
...
sub process_quotes {
    ...
    foreach my $quote (@$quotes) {
        ...
        update_quote_status( $quote->{logged_message_id},
